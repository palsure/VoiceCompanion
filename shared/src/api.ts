/**
 * Shared API service configuration
 * This module provides a factory function to create platform-specific API clients
 */
...
/**
 * API service class that can be used by both mobile and web
 */
export class VoiceCompanionApi {
  ...
  async getRealtimeGuidance(
    ...
    const response = await this.api.post('/guidance/realtime', {
      imageData,
      previousContext,
    })
    return response.data
  ...
  // Shopping Assistance API
  async getShoppingAssistance(
    imageData: string,
    question?: string
  ): Promise<any> {
    const response = await this.api.post('/daily-living/shopping-assist', {
      imageData,
      question,
    })
    return response.data
  ...
  async saveToGallery(image: string, prompt: string, style: string, userId?: string): Promise<any> {
    const response = await this.api.post('/gallery/save', {
      image,
      prompt,
      style,
      userId,
  ...
  async getGallery(userId?: string): Promise<any> {
    const response = await this.api.get('/gallery/list', {
      params: { userId },
    })
    return response.data
  }
  ...
  async getArtById(id: string, userId?: string): Promise<any> {
    const response = await this.api.get(`/gallery/${id}`, {
      params: { userId },
    })
    return response.data
  }
...
/**
 * Convenience functions that match the original API structure
 */
export function createApiServices(apiClient: AxiosInstance) {
  ...
  return {
    ...
    feedbackApi: {
      analyze: (text: string, audioData?: string) =>
        api.analyzeFeedback(text, audioData),
    },
    progressApi: {
      get: () => api.getProgress(),
    ...
    shoppingApi: {
      assist: (imageData: string, question?: string) =>
        api.getShoppingAssistance(imageData, question),
    },
    textToSpeechApi: {
